            ...
            <div className="w-full pt-[26px] 2xl:pt-[34px] ">
                ...
                <div className='w-full h-fit min-h-[300px] max-h-[420px] relative rounded-xl overflow-hidden flex flex-col items-start justify-center' >
                    ...
                    <div className='z-10 mt-20 absolute top-1/2 transform -translate-y-1/2 p-10' >
                        ...
                        <h3 className='text-white text-[28px] 2xl:text-[32px] ' >{currentCourse?.title}</h3>
                    </div>
                </div>
                ...
                <div className="w-full bg-brownGray py-[13px] px-[26px] rounded-2xl mt-6 flex items-center gap-[28px]">
                    ...
                    <div className="flex-[1] flex flex-col items-start">
                        <div className="w-full bg-brownerGray h-[14px] rounded-full mb-2 overflow-hidden">
                            <div style={{
                            width:`${ currentCourse ? Math.floor( currentCourse?.completed / currentCourse?.total * 100) : 0}%`
                            }} className="bg-primary w-1/2 h-full rounded-full" />
                        </div>
                        <span className='text-primary text-[11px] 2xl:text-[13px] font-[100] ' >{currentCourse ? Math.floor( currentCourse?.completed / currentCourse?.total * 100) : 0}% Completed</span>
                    </div>
                    ...
                    <div className='hidden lg:block'>
                        <button onClick={() => router.push(`/dashboard/courses?id=${currentCourse?.id}&watch=${currentCourse?.videos}`)} className='buttons-2 flex items-center gap-1 !px-16' >
