...
export const InputFade: FC<emailInputPropsFade> = (props) => {
  return (
    <div className="input-wrap">
      ...
      />
      {props.error && (
        <p className="text-error text-[10px] italic">{props.error}</p>
      )}
    </div>
  );
};
...
export const SearchInputFade: FC<searchInputProps> = (props) => {
  ...
  return (
    <div className="input-wrap">
      <div className="password-input">
        <input
          className="inputsfade min-w-[260px] 2xl:min-w-[460px] "
          type="text"
          placeholder={props.placeholder && props.placeholder}
        />
        <div
          onClick={handleClick}
          className="absolute cursor-pointer right-[20px] top-1/2 transform -translate-y-1/2 flex items-center justify-center text-icons"
        >
