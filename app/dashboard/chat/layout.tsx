...
const layout = ({ children }: { children: React.ReactNode }) => {
  ...
  const { message, setMessage, messages, setMessages, chatId, setChatId } =
  ...
  const scrollElement = document.getElementById("scrollToBottom");

  function scrollToBottom() {
    var bottomElement = scrollElement?.lastElementChild;
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    console.log(
      " This is what we sent oh...: ",
      chatData as unknown as ArrayBuffer
    );
  ...
  useEffect(() => {
    ...
    socket.onmessage = (data: any) => {
      setMessages((prevMessages: messagesData[] | null) => {
        return [...(prevMessages as messagesData[]), JSON.parse(data?.data)];
      });
      // alert('a new message has arrived')
      console.log("this is the onmessage data: ", data.data);
      console.log("this is the newMessages data: ", messages);
    };

    socket.onerror = (err) => {
      console.log("this is the error message from the socket: ", err);
    };
      ...
      <div className="w-full flex-[1] max-h-full flex items-start relative ">
        ...
        <div className="flex-[5] h-full">
          ...
          <div className="w-full flex items-center justify-center pt-2">
            {isOpen ? (
              <p className="text-[10px] text-success bg-successTrans10 py-1 px-4 rounded-md">
                Online
              </p>
            ) : (
              <p className="text-[10px] text-error2 bg-error2Trans10 py-1 px-4 rounded-md ">
                Offline
              </p>
            )}
          </div>
          <div className="py-3 w-[94%] mx-auto mt-[20px] 2xl:mt-[30px] rounded-2xl bg-white px-[20px] 2xl:px-[28px] pr-[10px] 2xl:pr-[14px] h-[90%]  relative">
            ...
            <div
              id="scrollToBottom"
              className="w-full h-[90%] scroll-chat mb-6 pr-2"
            >
              {children}
            </div>
              ...
              <div className="w-full bg-white p-1 rounded-2xl flex items-center gap-[14px] 2xl:gap-[16px]">
                ...
                <form onSubmit={handleSubmit} className="w-full relative ">
                  <input
                    ...
                    className="w-full rounded-full bg-chatinput border-2 border-chatinput p-[12px] text-[12px] focus:border-2 focus:border-outbox"
                  />
                  <button
                    type="submit"
                    className="w-[30px] h-[30px] p-1 rounded-full 2xl:w-[30px] 2xl:h-[30px] absolute right-[30px] top-1/2 transform -translate-y-1/2 hover:bg-sidebarTxtHover active:bg-blueChatHighlightActive "
                  >
                    <Image
